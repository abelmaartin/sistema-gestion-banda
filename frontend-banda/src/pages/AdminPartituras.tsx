    ...
    <div className="min-h-screen bg-slate-50 p-6 md:p-12">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Gestor General de Archivos</h1>
            <p className="text-slate-500 text-sm">Panel de Administración</p>
          </div>
          <div className="flex gap-4">
            {/* Nuevo botón para dar de alta usuarios */}
            <button 
              onClick={() => navigate('/admin/registrar')} 
              className="px-5 py-2.5 bg-emerald-600 text-white font-medium rounded-xl shadow-sm hover:bg-emerald-700 transition-colors"
            >
              + Registrar Miembro
            </button>

            <button onClick={() => navigate('/admin/usuarios')} className="px-5 py-2.5 bg-sky-600 text-white font-medium rounded-xl shadow-sm hover:bg-sky-700 transition-colors">
              👥 Gestionar Usuarios
            </button>
            ...
            <button onClick={() => navigate('/panel')} className="px-5 py-2.5 bg-slate-100 text-slate-700 rounded-xl hover:bg-slate-200 transition-colors">
